...
def my_decks(update, context):
    ...
    if len(text) == 1:
        ...
        bot.sendMessage(chat_id, 'Which do you want to add', reply_markup = reply_markup)
        return 'CHOICE'
    elif len(text) == 2:
        button = KeyboardButton(str(text[0]))
...
def alarm(context):
    ...
    button1 = InlineKeyboardButton('Show answer', callback_data='show_answer_time')
...
def alarm_minut(update, context):
    ...
    tank = tim(dt.year, dt.month, dt.day, dt.hour, dt.minute+hour, lst[x], name)
    ...
    if tank:
        context.job_queue.run_once(alarm, datetime.timedelta(minutes=hour), context=chat_id, name=str(first))
    if len(lst) == x+1:
        print('munit if')
...
def day(update, context):
    ...
    print(l, x)
    if tank:
        context.job_queue.run_once(alarm, datetime.timedelta(days=hour), context=chat_id, name=str(first))
    if l == x-1:
        print('munit if')
        button = InlineKeyboardButton('Hide', callback_data='hide')
        reply_markup = InlineKeyboardMarkup([[button]])
...
def close(update, context):
    ...
    update.message.reply_text('Thank you! Don\'t forget to memorized all of them', reply_markup = button)
...
updater.dispatcher.add_handler(CallbackQueryHandler(show_alarm_answer, pattern='show_answer_time'))
updater.dispatcher.add_handler(CallbackQueryHandler(alarm_minut, pattern='a1'))
updater.dispatcher.add_handler(CallbackQueryHandler(alarm_minut, pattern='a10'))
updater.dispatcher.add_handler(CallbackQueryHandler(show_answer, pattern='show_answer'))
...
updater.dispatcher.add_handler(MessageHandler(Filters.text('Show answer'), show_answer))
